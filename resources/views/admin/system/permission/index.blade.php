    ...
    <div class="layui-card">
        <div class="layui-card-header layuiadmin-card-header-auto">
            <div class="layui-btn-group ">
                @can('admin.system.permission.destroy')
                    <button class="layui-btn layui-btn-sm layui-btn-danger" id="listDelete">删 除</button>
                @endcan
                @can('admin.system.permission.create')
                    <a class="layui-btn layui-btn-sm" href="{{ route('admin.system.permission.create') }}">添 加</a>
                @endcan
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="options">
                <div class="layui-btn-group">
                    @can('admin.system.permission.index')
                        <a class="layui-btn layui-btn-sm" lay-event="children">子权限</a>
                    @endcan
                    @can('admin.system.permission.edit')
    ...
    <script>
        layui.use(['layer','table','form'],function () {
            ...
            table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'del'){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('admin.system.permission.destroy') }}",{_method:'delete',ids:[data.id]},function (result) {
                            if (result.code==0){
                    ...
                    location.href = "{{ route('admin.system.permission.index') }}/"+data.id+'/edit';
            ...
            $("#listDelete").click(function () {
                ...
                if (ids.length>0){
                    layer.confirm('确认删除吗？', function(index){
                        $.post("{{ route('admin.system.permission.destroy') }}",{_method:'delete',ids:ids},function (result) {
                            if (result.code==0){
                                dataTable.reload()
                            }
                            layer.close(index);
